...
CONFIG_FILE=$TRAVIS_BUILD_DIR/src/config/oasis-config.h
...
if [[ $HOST = "x86_64-apple-darwin11" ]]; then
    find $TRAVIS_BUILD_DIR -type f | grep -i oasis-Coin.dmg$ | xargs -i cp {} $OUTDIR/bin
else
    cp $TRAVIS_BUILD_DIR/src/qt/oasis-qt $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/qt/oasis-qt.exe $OUTDIR/bin/ || echo "no QT Wallet"
    cp $TRAVIS_BUILD_DIR/src/oasisd $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/oasis.exe $OUTDIR/bin/
    cp $TRAVIS_BUILD_DIR/src/oasis-cli $OUTDIR/bin/ || cp $TRAVIS_BUILD_DIR/src/oasis-cli.exe $OUTDIR/bin/
